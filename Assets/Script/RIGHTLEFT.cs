...
public class RIGHTLEFT : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            bg1.position = new Vector3(bg1.position.x + size, bg2.position.y, bg1.position.z);
            SwichingBg();
        }
        if (transform.position.x < bg1.position.x)
        {
            bg2.position = new Vector3(bg2.position.x - size, bg1.position.y, bg2.position.z);
            SwichingBg();
        }
    }
